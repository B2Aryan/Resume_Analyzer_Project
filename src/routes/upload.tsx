...
function UploadPage() {
  ...
  const [analysisStep, setAnalysisStep] = useState<AnalysisProgressStepId>("parsing");
  ...
  const handleAnalyze = useCallback(() => {
    ...
    if (uploadMethod === "file") {
      ...
      reader.onload = () => {
        ...
        startAnalysis(text, role, fileName);
      };
      ...
      startAnalysis(trimmed, role, fileName);
    }

    async function startAnalysis(text: string, targetRole: string, name: string) {
      startTransition(async () => {
        try {
          const result = await runResumeAnalysis(text, targetRole, {
            onStepChange: setAnalysisStep,
          });
          setResult(result, targetRole, name, text, pastedRole, {
            animateEntry: true,
          });
          navigate({ to: "/result" });
        } catch {
